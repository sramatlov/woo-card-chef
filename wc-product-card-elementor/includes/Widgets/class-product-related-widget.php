<?php
/**
 * PDP Product Cross-sells / Related widget for Elementor.
 *
 * Renders WooCommerce cross-sells for the current product, falling back to
 * WooCommerce related products when no visible cross-sells are available. Cards
 * use the same shared renderer as the archive Product Card Grid and Product
 * Upsells widget.
 *
...
/**
 * PDP Product Cross-sells / Related Elementor widget.
 *
 * @since 2.6.0
 */
class WCPCE_Product_Related_Widget extends \Elementor\Widget_Base {
	...
	public function get_title(): string {
		return esc_html__( 'Product Cross-sells / Related (PDP)', 'woo-card-chef' );
	}
	...
	public function get_keywords(): array {
		return array( 'cross-sells', 'cross sells', 'related', 'recommendations', 'aanbevolen', 'product', 'woocommerce', 'pdp' );
	}
	...
	public function get_style_depends(): array {
		return array( 'wc-product-card-elementor', 'wcpce-product-related' );
	}
	...
	private function register_content_controls(): void {
		$this->start_controls_section(
			...
			array(
				'label' => esc_html__( 'Cross-sells / Related', 'woo-card-chef' ),
		...
		$this->add_control(
			...
			array(
				...
				'default'     => __( 'Ook interessant', 'woo-card-chef' ),
				'placeholder' => esc_html__( 'Ook interessant', 'woo-card-chef' ),
		...
		$this->add_control(
			...
			array(
				...
				'min'         => 1,
				'max'         => 24,
				'description' => esc_html__( 'Uses WooCommerce cross-sells first. If none are visible, falls back to WooCommerce related products.', 'woo-card-chef' ),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => \Elementor\Controls_Manager::TEXT,
				'default'   => __( 'Geen gerelateerde producten gevonden.', 'woo-card-chef' ),
				'ai'        => array( 'active' => false ),
	...
	private function register_layout_style_controls(): void {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'max'            => 6,
				'selectors'      => array(
					'{{WRAPPER}} .wcpce-related__grid' => 'grid-template-columns: repeat({{VALUE}}, 1fr);',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'      => array(
					'{{WRAPPER}} .wcpce-related__grid' => 'gap: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'max'            => 5,
				'selectors'      => array(
					'{{WRAPPER}} .wcpce-related .wc-card__title' => '-webkit-line-clamp: {{VALUE}}; min-height: calc(1em * 1.28 * {{VALUE}});',
				),
			)
		);
	...
	private function register_section_style_controls(): void {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .wcpce-related' => 'gap: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .wcpce-related__heading',
				'condition' => array( 'heading_text!' => '' ),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#d45550',
				'selectors' => array(
					'{{WRAPPER}} .wcpce-related__heading' => 'color: {{VALUE}};',
				),
				'condition' => array( 'heading_text!' => '' ),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'default'   => 'left',
				'selectors' => array(
					'{{WRAPPER}} .wcpce-related__heading' => 'text-align: {{VALUE}};',
				),
				'condition' => array( 'heading_text!' => '' ),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .wcpce-related__heading' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
				'condition'  => array( 'heading_text!' => '' ),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .wcpce-related__heading' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
				'condition'  => array( 'heading_text!' => '' ),
				'separator'  => 'before',
			)
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '',
				'selectors' => array(
					'{{WRAPPER}} .wcpce-related__heading' => 'background-color: {{VALUE}};',
				),
				'condition' => array( 'heading_text!' => '' ),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .wcpce-related__heading' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
				'condition'  => array( 'heading_text!' => '' ),
			)
		);
	...
	private function register_typography_controls(): void {
		...
		$this->add_group_control(
			...
			array(
				...
				'selector'       => '{{WRAPPER}} .wcpce-related .wc-card__title',
		...
		$this->add_group_control(
			...
			array(
				...
				'selector'       => '{{WRAPPER}} .wcpce-related .wc-card__price-current, {{WRAPPER}} .wcpce-related .wc-card__price-sale',
				'separator'      => 'before',
				'fields_options' => array(
					'typography'  => array(
						'default' => 'custom',
	...
	private function register_color_controls(): void {
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#2a2a2a',
				'selectors' => array(
					'{{WRAPPER}} .wcpce-related .wc-card__title' => 'color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .wcpce-related .wc-card__stock-label' => 'background-color: {{VALUE}};',
				),
				'condition' => array( 'show_out_of_stock_label' => 'yes' ),
				'separator' => 'before',
			)
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#ffffff',
				'selectors' => array(
					'{{WRAPPER}} .wcpce-related .wc-card__stock-label' => 'color: {{VALUE}};',
				),
				'condition' => array( 'show_out_of_stock_label' => 'yes' ),
			)
		);
	 ...
	 */
	private function validate_related_settings( array $settings ): array {
		...
		$text_defaults = array(
			'heading_text'                 => __( 'Ook interessant', 'woo-card-chef' ),
			'empty_state_text'             => __( 'Geen gerelateerde producten gevonden.', 'woo-card-chef' ),
	 ...
	 *
	 * @since 2.6.0
	 * @since 2.6.0 Prefer queried product before global product.
	...
	/**
	 * Returns visible WooCommerce cross-sells, or related products as fallback.
	 *
	 * @since 2.6.0
	 ...
	 */
	private function get_recommendation_products( \WC_Product $product, int $max ): array {
		$cross_sells = $this->get_visible_products_from_ids( $product->get_cross_sell_ids( 'view' ), $product->get_id(), $max );
		...
		$related_limit = max( $max, $max * 3 );
		$related_ids   = wc_get_related_products( $product->get_id(), $related_limit, array( $product->get_id() ) );

		return $this->get_visible_products_from_ids( $related_ids, $product->get_id(), $max );
	}

	/**
	 * Maps product IDs to visible product objects while preserving ID order.
	 *
	 * @since 2.6.0
	 * @param array<int,mixed> $ids        Candidate product IDs.
	 * @param int              $current_id Current product ID to exclude.
	 * @param int              $max        Maximum products.
	 * @return array<int,\WC_Product>
	 */
	private function get_visible_products_from_ids( array $ids, int $current_id, int $max ): array {
		$ids = array_values(
			array_unique(
				array_filter(
		...
		$visible = array();
		foreach ( $ids as $id ) {
			// Named $candidate (not $product) so this local can never be
			// confused with the WooCommerce global — global $product pollution
			// by product loops is exactly what v2.5.1/v2.5.8 guarded against.
			$candidate = wc_get_product( $id );
			if ( ! $candidate instanceof \WC_Product ) {
				continue;
			}

			if ( function_exists( 'wc_products_array_filter_visible' ) && ! wc_products_array_filter_visible( $candidate ) ) {
				continue;
			}

			if ( ! function_exists( 'wc_products_array_filter_visible' ) && ! $candidate->is_visible() ) {
				continue;
			}

			$visible[] = $candidate;
			if ( count( $visible ) >= $max ) {
				break;
			}
		}
	...
	private function render_editor_notices( ?\WC_Product $product, array $products ): void {
		...
		if ( null === $product ) {
			echo '<div class="wcpce-related-editor-notice">';
		...
		if ( empty( $products ) ) {
			echo '<div class="wcpce-related-editor-notice">';
			echo esc_html__( 'This product has no visible WooCommerce cross-sells or related products.', 'woo-card-chef' );
			echo '</div>';
		}
	}
	...
	private function render_empty_state( array $settings ): void {
		$message = $settings['empty_state_text'] ?? __( 'Geen gerelateerde producten gevonden.', 'woo-card-chef' );

		echo '<div class="wcpce-related__empty">';
	...
	protected function render(): void {
		...
		$settings  = $this->validate_related_settings( $this->get_settings_for_display() );
		...
		$products = null === $product ? array() : $this->get_recommendation_products( $product, (int) $settings['max_items'] );
		...
		$classes       = array( 'wcpce-related', 'wcpce-grid-section' );

		if ( 'scroll' === ( $settings['mobile_layout'] ?? 'scroll' ) ) {
			$classes[] = 'wcpce-related--mobile-scroll';
		}
		...
		if ( '' !== ( $settings['heading_text'] ?? '' ) ) {
			...
			echo '<' . tag_escape( $tag ) . ' class="wcpce-related__heading">' . esc_html( $settings['heading_text'] ) . '</' . tag_escape( $tag ) . '>';
		}
		...
		foreach ( $products as $related ) {
			echo '<li class="product wcpce-grid-item wcpce-related__item" role="listitem">';
			WCPCE_Card_Renderer::render_card( $related, $card_settings, $index, $widget_id );
			echo '</li>';
			$index++;
		}
